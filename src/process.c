...
//***************************************************************************
static Total_Ticket = 0;
int ticket[Num_Ticket];
static num_p = 1; 
int tn[15] = {150, 120, 100, 90, 80, 70, 60, 50, 40, 30, 20, 10, 4, 2, 1};
...
ProcessSetResult (PCB * pcb, uint32 result)
{
 ...
 //      LotterySchedule                                     
 //                                               
 //      Schedule the next process to run randomly.      
 //                                                           
 //----------------------------------------------------------------------   
...
ProcessSchedule ()
{
  ...
  total_num_quanta++;
  runTime += my_timer_get() - startTime;
...
ProcessFork (VoidFunc func, uint32 param, int p_nice, int p_info,char *name, int isUser)
{
  int		i, j, fd, n, pos, b;
  Link		*l;
  int		start, codeS, codeL, dataS, dataL, nTicket;
  uint32	*stackframe;
  int		newPage;
  PCB		*pcb;
  int	addr = 0;
  ...
  if(currentPCB != NULL) {
    b = Total_Ticket;
    Total_Ticket += tn[num_p-1];
    
    for(pos=b; pos < Total_Ticket; pos++) 
      {
	ticket[pos] = pcb->p_nice;
      }

    //printf("Ticket = %d\t", tn[num_p-1]);
